...
CREATE TABLE `_DepartmentToSubject` (
    ...
    UNIQUE INDEX `_DepartmentToSubject_AB_unique`(`A`, `B`),
    INDEX `_DepartmentToSubject_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `_ClassroomToSubject` (
    ...
    UNIQUE INDEX `_ClassroomToSubject_AB_unique`(`A`, `B`),
    INDEX `_ClassroomToSubject_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `Department` ADD CONSTRAINT `Department_examId_fkey` FOREIGN KEY (`examId`) REFERENCES `Exam`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
